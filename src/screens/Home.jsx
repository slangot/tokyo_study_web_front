...
export default function Home() {
  ...
  const userName = sessionStorage.getItem("user_name").replaceAll("\"", "").toLowerCase()
  const userPlanGrade = sessionStorage.getItem("user_plan_grade").replaceAll("\"", "").toLowerCase()
  const userTokens = sessionStorage.getItem("user_tokens").replaceAll("\"", "").toLowerCase()
    ...
    <article className="relative flex flex-col justify-evenly pt-10 min-h-[90dvh]">
      ...
      <section className="flex flex-col mx-5 w-auto">
        ...
        <div className="flex flex-row w-auto overflow-scroll hide-scrollbar gap-2">
          ...
          <SliderButtons isLocked={userPlanGrade !== "premium"} link={"/exercices/hiddenwords"} text={"Mots cachés"} />
          <SliderButtons isLocked={userPlanGrade !== "premium"} link={"/exercices/story"} text={"Histoire"} />
